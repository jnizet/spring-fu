...
open class CoroutineRouterFunctionDsl(private val init: (CoroutineRouterFunctionDsl.() -> Unit)): () -> RouterFunction<ServerResponse> {
    ...
    fun <T: CoroutineServerResponse> pathExtension(extension: String, f:  suspend (CoroutineServerRequest) -> CoroutineServerResponse?) {
        routes += route(RequestPredicates.pathExtension(extension), f.asHandlerFunction())
    }

    fun <T: CoroutineServerResponse> POST(pattern: String, f:  suspend (CoroutineServerRequest) -> CoroutineServerResponse?) {
    ...
    operator fun <T: CoroutineServerResponse> RequestPredicate.invoke(f:  suspend (CoroutineServerRequest) -> CoroutineServerResponse?) {
